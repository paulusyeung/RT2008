...
namespace RT2008.Staff
{
    public partial class FindHKID : Form
    {
        ...
        private void btnFind_Click(object sender, EventArgs e)
        {
            ...
            RT2008.DAL.Staff staff = null;
            if (!txtStaff.Text.Trim().Equals("*"))
            {
                string whereStr = " StaffNumber = '" + txtStaff.Text.Trim() + "'";
                staff = RT2008.DAL.Staff.LoadWhere(whereStr);
            }

            RT2008.DAL.SmartTag4Staff smartTag4Staff = RT2008.DAL.SmartTag4Staff.LoadWhere(" TagCode = 'HKID'");
            ...
            RT2008.DAL.StaffSmartTagCollection oStaffSmartTag = RT2008.DAL.StaffSmartTag.LoadCollection(whereClause);

            if (oStaffSmartTag.Count > 0)
            {
                int iCount = 1;
                foreach (RT2008.DAL.StaffSmartTag staffSmartTag in oStaffSmartTag)
                {
                    ListViewItem objItem = this.lvStaffList.Items.Add(iCount.ToString());
                    if (staff == null)
                    {
                        RT2008.DAL.Staff sta = RT2008.DAL.Staff.Load(staffSmartTag.StaffId);
                        if (sta != null)
                        {
