    ...
    public partial class ConfirmationWizard : Form, IGatewayComponent
    {
        ...
        private void LoadTxferInfo()
        {
            ...
            if (oHeader != null)
            {
                ...
                txtLatestConfirmedOn.Text = RT2008.SystemInfo.Settings.DateTimeToString(oHeader.CONFIRM_TRF_LASTUPDATE, false);
        ...
        {
            RT2008.DAL.Workplace wp = RT2008.DAL.Workplace.LoadWhere("WorkplaceCode = '" + wpcode + "'");
            if (wp != null)
            {
        ...
        private void SaveMessageHandler(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (Save())
                {
                    ...
                    this.Close();
                    RT2008.Inventory.Transfer.ConfirmationWizard wizard = new RT2008.Inventory.Transfer.ConfirmationWizard(this.TxferId);
                    wizard.ShowDialog();
                }
            }
        }
