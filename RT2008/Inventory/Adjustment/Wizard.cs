...
namespace RT2008.Inventory.Adjustment
{
    public partial class Wizard : Form, IGatewayComponent, IWizard
    {
        ...
        void IGatewayComponent.ProcessRequest(IContext objContext, string strAction)
        {
            ...
            RT2008.Inventory.Adjustment.Reports.WorksheetRpt report = new RT2008.Inventory.Adjustment.Reports.WorksheetRpt();
        ...
        private void cmdPreview_Click()
        {
            ...
            oViewer.ReportName = "RT2008.Inventory.Adjustment.Reports.WorksheetRdl.rdlc";
            oViewer.ReportDatasourceName = "RT2008_Controls_Reporting_DataSource_vwRptBatchADJ";
            ...
            if (errorProvider.GetError(cboWorkplace).Length == 0 && errorProvider.GetError(cboOperatorCode).Length == 0)
            {
                if (lvDetailsList.Items.Count > 0)
                {
                    ...
                    if (oHeader == null)
                    {
                        ...
                        txtTxNumber.Text = RT2008.SystemInfo.Settings.QueuingTxNumber(Common.Enums.TxType.ADJ);
            ...
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    ...
                    if (this.ADJId != System.Guid.Empty)
                    {
                        ...
                        this.Close();
                        RT2008.Inventory.Adjustment.Wizard wizard = new RT2008.Inventory.Adjustment.Wizard(this.ADJId);
                        wizard.ShowDialog();
                    }
                }
                else
            ...
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    ...
                    if (this.ADJId != System.Guid.Empty)
                    {
                        RT2008.SystemInfo.Settings.RefreshMainList<Default>();
                        this.Close();
                        RT2008.Inventory.Adjustment.Wizard wizard = new RT2008.Inventory.Adjustment.Wizard();
                        wizard.ShowDialog();
                    }
                }
                else
        ...
        private void basicFindProduct_SelectionChanged(object sender, RT2008.Controls.ProductSearcher.Basic.ProductSelectionEventArgs e)
        {
            if (!this.ValidSelection)
            {
                ...
                txtOnHand.Text = RT2008.Controls.Utility.GetOnHandQtyByWorkplaceId(e.ProductId, "'" + cboWorkplace.SelectedValue.ToString() + "'").ToString("n0");
