...
namespace RT2008.Inventory.StockTake.Reports
{
    public partial class VarianceList : Form
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            dtpFromDate.Value = Convert.ToDateTime(RT2008.SystemInfo.CurrentInfo.Default.CurrentSystemDate.ToString("yyyy-MM-01"));
        ...
        private void Init()
        {
            switch (this.ReportType)
            {
                case VarianceReportType.Worksheet:
                    ...
                    reportName = "RT2008.Inventory.StockTake.Reports.VarianceListWorksheetRdl.rdlc";
                    break;
                case VarianceReportType.History:
                    this.Text = "Stock Take Variance List History";
                    whereClause = "YEAR(PostedOn) > 1900";
                    reportName = "RT2008.Inventory.StockTake.Reports.VarianceListHistoryRdl.rdlc";
                    break;
                case VarianceReportType.WorksheetWithoutCost:
                    this.Text = "Stock Take Variance List (Without COST)";
                    whereClause = "YEAR(PostedOn) = 1900";
                    reportName = "RT2008.Inventory.StockTake.Reports.VarianceListWorksheetWitoutCostRdl.rdlc";
                    break;
                case VarianceReportType.HistoryWithoutCost:
                    this.Text = "Stock Take Variance List History (Without COST)";
                    whereClause = "YEAR(PostedOn) > 1900";
                    reportName = "RT2008.Inventory.StockTake.Reports.VarianceListHistoryWitoutCostRdl.rdlc";
                    break;
            }
        ...
        private void btnPreview_Click(object sender, EventArgs e)
        {
            if (IsSelValid() && reportName.Trim().Length > 0)
            {
                ...
                { "FromTxDate",this.dtpFromDate.Value.ToString(RT2008.SystemInfo.Settings.GetDateFormat())},
                { "ToTxDate",this.dtpToDate.Value.ToString(RT2008.SystemInfo.Settings.GetDateFormat())},
