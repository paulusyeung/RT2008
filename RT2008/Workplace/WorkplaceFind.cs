...
namespace RT2008.Workplace
{
    public partial class WorkplaceFind : Form
    {
        ...
        private void btnFind_Click(object sender, EventArgs e)
        {
            ...
            RT2008.DAL.WorkplaceCollection WorkplaceCol;
            if (whereClause != string.Empty)
            {
                WorkplaceCol = RT2008.DAL.Workplace.LoadCollection(whereClause);
            }
            else
            {
                WorkplaceCol = RT2008.DAL.Workplace.LoadCollection();   
            }

            if (WorkplaceCol.Count > 0)
            {
                int iCount = 1;
                foreach (RT2008.DAL.Workplace workplace in WorkplaceCol)
                {
