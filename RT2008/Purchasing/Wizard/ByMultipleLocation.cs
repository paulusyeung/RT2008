...
namespace RT2008.Purchasing.Wizard
{
    ...
    public partial class ByMultipleLocation : Form
    {
        ...
        {
            RT2008.DAL.Supplier.LoadCombo(ref this.cboSupplierCode, new string[] { "SupplierCode", "SupplierInitial" }, "{0} - {1}", false, false, string.Empty, string.Empty, null);
        }
        ...
        {
            RT2008.DAL.Staff.LoadCombo(ref this.cboOperatorCode, new string[] { "StaffNumber", "FullName" }, "{0} - {1}", false, false, string.Empty, string.Empty, null);
        }
        ...
        {
            RT2008.DAL.Currency.LoadCombo(ref this.cboCurrency, "CurrencyCode", false);
        }
        #endregion
        ...
        private void Save()
        {
            ...
            foreach (ListViewItem objItem in this.lvLocationsList.Items)
            {
                if (objItem.SubItems[1].Text.Contains("*"))
                {
                    ...
                    {
                        case "FPO":
                            orderNumber = RT2008.SystemInfo.Settings.QueuingTxNumber(Common.Enums.POType.FPO);
                            type = Convert.ToInt32(Common.Enums.POType.FPO);
                            break;
                        case "LPO":
                            orderNumber = RT2008.SystemInfo.Settings.QueuingTxNumber(Common.Enums.POType.LPO);
                            type = Convert.ToInt32(Common.Enums.POType.LPO);
                            break;
                        case "OPO":
                            orderNumber = RT2008.SystemInfo.Settings.QueuingTxNumber(Common.Enums.POType.OPO);
                    ...
                    objHeader.SupplierId = this.cboSupplierCode.SelectedIndex == -1 ? System.Guid.Empty : RT2008.Purchasing.PurchasingUtils.Convert.ToGuid(this.cboSupplierCode.SelectedValue.ToString());
                    objHeader.StaffId = RT2008.Purchasing.PurchasingUtils.Convert.ToGuid(this.cboOperatorCode.SelectedValue.ToString());
                    ...
                    objHeader.WorkplaceId = RT2008.Purchasing.PurchasingUtils.Convert.ToGuid(objItem.SubItems[0].Text);
                    objHeader.CurrencyCode = this.cboCurrency.Text;
                    objHeader.Status = RT2008.Purchasing.PurchasingUtils.Convert.ToInt32(this.cboStatus.Text == "HOLD" ? Common.Enums.Status.Draft.ToString("d") : Common.Enums.Status.Active.ToString("d"));
        ...
        private void SaveOrderDetail(PurchaseOrderHeader header)
        {
            foreach (ListViewItem listItem in this.lvDetailsList.Items)
            {
                ...
                {
                    System.Guid detailId = RT2008.Purchasing.PurchasingUtils.Convert.ToGuid(listItem.Text.Trim());
                    ...
                    if (objDetail == null)
                    {
                        ...
                        objDetail.LineNumber = RT2008.Purchasing.PurchasingUtils.Convert.ToInt32(listItem.SubItems[1].Text.Length == 0 ? "1" : listItem.SubItems[1].Text);
                    }

                    objDetail.ProductId = RT2008.Purchasing.PurchasingUtils.Convert.ToGuid(listItem.SubItems[8].Text.Trim());
                    for (int i = 9; i <= 18; i++)
                    {
                        if (workplace.WorkplaceCode == this.lvDetailsList.Columns[i].Text && listItem.SubItems[i].Text != "0")
                        {
                            objDetail.OrderedQty = RT2008.Purchasing.PurchasingUtils.Convert.ToDecimal(listItem.SubItems[i].Text.Length == 0 ? "0" : listItem.SubItems[i].Text);
                            result = true;
                            break;
                        }
                        else
                    ...
                    objDetail.UnitCost = RT2008.Purchasing.PurchasingUtils.Convert.ToDecimal(listItem.SubItems[7].Text.Length == 0 ? "0" : listItem.SubItems[7].Text);
        ...
        private void ItemInfo(ref string stkCode, ref string appendix1, ref string appendix2, ref string appendix3)
        {
            ...
            {
                RT2008.DAL.Product objProd = RT2008.DAL.Product.Load(RT2008.Purchasing.PurchasingUtils.Convert.ToGuid(this.basicProduct.SelectedItem.ToString()));
                if (objProd != null)
                {
