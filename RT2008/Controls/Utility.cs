...
namespace RT2008.Controls
{
    public class Utility
    {
        ...
        public static string GetWorkplaceIdListByZone(Guid zoneId)
        {
            ...
            WorkplaceCollection wpList = RT2008.DAL.Workplace.LoadCollection(sql);
        ...
        {
            string label = RT2008.Controls.Utility.Dictionary.GetWord("Last_days");
            ...
            cboView.Items.Add(RT2008.Controls.Utility.Dictionary.GetWord("All"));
        }
        ...
        public class Default
        {
            public static Color TopPanelBackgroundColor
            {
                get
                {
                    ...
                    if (HttpContext.Current.Request.Cookies["RT2008_TopPanelBackgroundColor"] != null)
                    {
                        color = HttpContext.Current.Request.Cookies["RT2008_TopPanelBackgroundColor"].Value == String.Empty ?
                            Color.FromName("#ACC0E9") :
                            Color.FromName(HttpContext.Current.Request.Cookies["RT2008_TopPanelBackgroundColor"].Value);
                    }
                    return color;
                }
                set
                {
                    System.Web.HttpCookie oCookie = new System.Web.HttpCookie("RT2008_TopPanelBackgroundColor");
            ...
            public static String CurrentTheme
            {
                ...
                {
                    String theme = "";
                    if (HttpContext.Current.Request.Cookies["RT2008_CurrentTheme"] != null)
                    {
                        theme = HttpContext.Current.Request.Cookies["RT2008_CurrentTheme"].Value;
                    }
                    return theme == String.Empty ? "Vista" : theme;
                }
                set
                {
                    System.Web.HttpCookie oCookie = new System.Web.HttpCookie("RT2008_CurrentTheme");
    ...
    public class InvtUtility
    {
        ...
        {
            RT2008.Controls.InvtTxSearcher searchForm = new RT2008.Controls.InvtTxSearcher();
    ...
    public class UserUtility
    {
        public static string PermissionLevel()
        {
            ...
            RT2008.DAL.Staff user = RT2008.DAL.Staff.Load(Common.Config.CurrentUserId);
            if (user != null)
            {
        ...
        public static bool DeleteRec(Guid userId)
        {
            ...
            RT2008.DAL.Staff user = RT2008.DAL.Staff.Load(userId);
            if (user != null)
            {
    ...
    public class Preference
    {
        ...
        public static void Load(ref ListView lvwList)
        {
            ...
            if (user != null)
            {
                ...
                sql = String.Format("UserId = '{0}' AND PreferenceObjectId = '{1}'", RT2008.Controls.UserProfile.GetSuperUserId().ToString(), ((Guid)lvwList.Tag).ToString());
                UserPreference userPref = UserPreference.LoadWhere(sql);
                if (userPref == null)
                {
                    sql = String.Format("UserId = '{0}' AND PreferenceObjectId = '{1}'", user.UserId.ToString(), ((Guid)lvwList.Tag).ToString());
                ...
                if (userPref != null)
                {
                    Dictionary<string, RT2008.DAL.UserPreference.MetadataAttributes> metadata = userPref.MetadataXml;
                    foreach (KeyValuePair<string, RT2008.DAL.UserPreference.MetadataAttributes> col in metadata)
                    {
                        int colIndex = int.Parse(col.Key);      // col.Key 等於 ColumnHeader.Index

                        foreach (RT2008.DAL.UserPreference.MetadataAttribute item in col.Value)
                        {
    ...
    public class Staff
    {
        public static bool DeleteRec(Guid userId)
        {
            ...
            RT2008.DAL.Staff staff = RT2008.DAL.Staff.Load(userId);
            if (staff != null)
            {
                // cannot delete primary user (primary user means CreatedBy = Guid.Empty)
                if (staff.CreatedBy != Guid.Empty)
                {
                    switch ((int)staff.Status)
                    {
                        ...
                        case (int)Common.Enums.Status.Draft:
                            RT2008.DAL.StaffAddressCollection addresses = RT2008.DAL.StaffAddress.LoadCollection(String.Format("StaffId = '{0}'", userId.ToString()));
                            if (addresses.Count > 0)
                            {
                                foreach (RT2008.DAL.StaffAddress address in addresses)
                                {
                                    address.Delete();
                                }
                            }
        ...
        public static Guid GetSuperStaffId()
        {
            ...
            RT2008.DAL.Staff staff = RT2008.DAL.Staff.LoadWhere(sql);
            if (staff != null)
            {
    ...
    public class UserProfile
    {
        public static bool SaveRec(Guid userSid, int userType, String loginName, String loginPassword, String alias)
        {
            ...
            {
                user = new RT2008.DAL.UserProfile();
