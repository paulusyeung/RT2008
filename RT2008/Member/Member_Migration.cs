...
namespace RT2008.Member
{
    public partial class Member_Migration : Form
    {
        ...
        {
            RT2008.DAL.MemberCollection objMemberList = RT2008.DAL.Member.LoadCollection();
            for (int i = 0; i < objMemberList.Count; i++)
            {
                RT2008.DAL.Member objMember = objMemberList[i];
        ...
        private Guid UpdateMemberMainInfo(MemberApply4TempVip objTempVip, bool canDelete)
        {
            ...
            RT2008.DAL.Member objMember = RT2008.DAL.Member.LoadWhere(query);
            if (objMember == null)
            {
                isNew = true;

                objMember = new RT2008.DAL.Member();
