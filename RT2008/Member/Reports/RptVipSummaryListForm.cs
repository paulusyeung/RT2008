...
namespace RT2008.Member.Reports
{
    public partial class RptVipSummaryListForm : Form
    {
        ...
        private void FillMemberNumber()
        {
            ...
            {
                foreach (RT2008.DAL.Member oItem in collection)
                {
        ...
        {
            MemberGroupCollection collection = RT2008.DAL.MemberGroup.LoadCollection(new string[] { "GroupName" }, true);
            if (collection.Count > 0)
            {
                foreach (RT2008.DAL.MemberGroup oItemGroup in collection)
                {
...
AND LEN(STKCODE)<>0)";
        ...
        private void btnPriview_Click(object sender, EventArgs e)
        {
            if (IsSelValid())
            {
                ...
                {
                    DateRange = this.dtpSpecifiedRangeFrom.Value.ToString(RT2008.SystemInfo.Settings.GetDateFormat()) + "-" +
                                this.dtpSpecifiedRangeTo.Value.ToString(RT2008.SystemInfo.Settings.GetDateFormat());
                }
                ...
                {"Appendix2",Appendix2},
                {"Appendix3",Appendix3},
                {"Appendix1Name",RT2008.SystemInfo.Settings.GetSystemLabelByKey("Appendix1")},
                {"Appendix2Name",RT2008.SystemInfo.Settings.GetSystemLabelByKey("Appendix2")},
                {"Appendix3Name",RT2008.SystemInfo.Settings.GetSystemLabelByKey("Appendix3")},
                ...
                if (rbtPDF.Checked)
                {
                    view.ReportName = "RT2008.Member.Reports.VipSummaryListFormRdl.rdlc";

                }
                else if (rbtXLS.Checked)
                {
                    view.ReportName = "RT2008.Member.Reports.VipSummaryListFormAppendix1..3Rdl.rdlc";
                }
                #endregion

                view.ReportDatasourceName = "RT2008_Controls_Reporting_DataSource_vwVIP_SalesSummary";
